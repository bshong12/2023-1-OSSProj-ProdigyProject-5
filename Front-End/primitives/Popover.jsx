...
function Footer() {
  return (
    <a
      href="#"
      tw="flow-root px-2 py-2 transition duration-150 ease-in-out rounded-md hover:bg-gray-100 focus:outline-none focus-visible:(ring ring-amber-500 ring-opacity-50)"
    >
      <span tw="flex items-center">
        <span tw="text-sm font-medium text-gray-900">Documentation</span>
      </span>
      <span tw="block text-sm text-gray-500">Start integrating products and tools</span>
    </a>
  )
}
function ContentItem(item) {
  ...
  const Illustration = Illustrations[item.name]
  return (
    <a
      key={item.name}
      href={item.href}
      tw="flex items-center p-2 -m-3 transition duration-150 ease-in-out rounded-lg hover:bg-gray-50 focus:outline-none focus-visible:(ring ring-amber-500 ring-opacity-50)"
    >
      {Illustration ? (
        <Fragment>
...
function Trigger({ label, open }) {
  return (
    <HeadlessPopover.Button
      className="group"
      css={[
        tw`text-white bg-amber-700 px-3 py-2 rounded-md inline-flex items-center text-base font-medium hover:text-opacity-100 focus:outline-none focus-visible:(ring ring-purple-500 ring-opacity-75)`,
    ...
    >
      ...
      <HiChevronDown
        css={[
          tw`ml-2 h-5 w-5 text-amber-300 group-hover:text-opacity-80 transition ease-in-out duration-150`,
